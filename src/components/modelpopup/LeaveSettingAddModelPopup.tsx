...
const LeaveSettingAddModelPopup = () => {
  ...
  const [optionsFrom, setOptionsFrom] = useState<any>(initialOptionsFrom);
  ...
  const moveRightSelected = () => {
    ...
    setOptionsFrom(
      optionsFrom.filter((option:any) => !selectedValues.includes(option.value))
    );
    setSelectedFrom([]);
  };
  ...
  const moveLeftSelected = () => {
    ...
    setOptionsFrom([
      ...optionsFrom,
      ...optionsTo.filter((option:any) => selectedValues.includes(option.value)),
    ]);
    setOptionsTo(
      optionsTo.filter((option:any) => !selectedValues.includes(option.value))
    );
              ...
              <form>
                ...
                <div className="input-block mb-3 leave-duallist">
                  ...
                  <div className="row">
                    ...
                    <div className="col-lg-5 col-sm-5">
                      <select
                        value={selectedTo.map((option:any) => option?.value)}
                        className="form-control form-select"
                        size={8}
                        multiple
                        onChange={(e) =>
                          setSelectedTo(
                            Array.from(e.target.selectedOptions, (item:any) =>
                              parseInt(item?.value)
                            )
                          )
                        }
                      >
                        {optionsTo.map((option:any) => (
                          <option key={option?.value} value={option?.value}>
                            {option.label}
                          </option>
                        ))}
